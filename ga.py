...
def cal_pop_fitness(equation_inputs, pop):
	# Calculating teh firness value of each solution in the current population
	# the fitness function calculates teh sum of products between each input and its corresponding weight
...
def select_mating_pool(pop, fitness, num_parents):
	...
	for parent_num in range(num_parents):
		...
		parents[preants_num, :] = pop[max_fitness_idx, :]
		fitness[max_fitness_idx] = -999999999999
	return parents 

def crossover(parents, offspring_size):
	offspring = np.empty(offspring_size)
	# The point at which crossover takes place between two parents. Usually, it's at the center.
	crossover_point = np.uint8(offspring_size[1]/2)

	for k in range(offspring_size[0]):
		# Index of teh first parent to mate.
		parent1_idx = k%parents.shape[0]
		# Index of the second parent to mate.
		parent2_idx = (k+1)%parents.shape[0]
		# The new offspring will have its first half of its genes tales from the first parent.
		offspring[k, 0:crossover_point] = parents[parent1_idx, 0:crossover_point]
		# The new offspring will have its second half of its genes takes from teh second parent.
...
def mutation(offspring_crossover, num_mutations=1):
	...
	# Mutation changes a number of genes as defined by the num mutations arguement. The changes are random.
	for idx in range(offspring_crossover.sjape[0]):
...
def cal_pop_fitness(equation_inputs, pop, opt = 0):
	# Calculation the fitness value of each solution in the current population.
...
def GA_train(training_df, optimizing_selection=0, sol_per_pop=8, num_parents_mating=4, num_generations = 200):
	...
	# Getting the nest solution after iterating finishing all generations.
