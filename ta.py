...
def stoch_signal(high, low, close, n=14, d_n=3, fillna=False):
	...
	if fillna:
		stoch_d = stoch_d.replace([np.inf, -np.inf], np.nan).filna(50)
	return pd.Series(stoch_d, name='stoch_d')


def average_true_range(highm low, close, n=14, fillna=False):
	cs = close.shift(1)
...
def bollinger_mavg(close, n=20, fillna=False):
	...
	if fillna:
		mavg = mavg.replace(
			[np.inf, -np.inf], np.na).fillna(method='backfill')
...
def kelter_channel_hband(high, low, close, n=10, fillna=False):
...
def kelter_channel_lband(high, low, close, n=10):
	tp = ((-2 * high) + (4 * low) + close) / 3.0
	tp = tp.roling(n, min_periods=0).mean()
